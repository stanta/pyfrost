...
class TestCaseKey(unittest.TestCase):
    def test_key(self):
        ...
        for key in sign_subset:
            ...
            for nonces in saved_data["private_data"][key.node_id].values():
                for nonce in nonces:
                    ...
                    if (
                        my_nonce_d_public == nonce_d_public
                        and my_nonce_e_public == nonce_e_public
                    ):
                        break
            nonce = {"nonce_d": nonce_d_private, "nonce_e": nonce_e_private}
            single_sign = key.sign(nonces_data, msg, nonce)
            signature_data = {
                "id": int(key.node_id),
                "message": msg,
                "nonces_dict": nonces_data,
                "aggregated_public_nonce": Utils.code_to_pub(
                    single_sign["aggregated_public_nonce"]
                ),
                "public_key_share": Utils.pub_to_code(
                    crypto.get_public_key(key.dkg_key_pair["share"], Utils.ecurve)
                ),
        ...
        self.assertEqual(len(set(agregated_nonces)), 1, "AGREGATED NONCES FAILED")

        group_sign = pyfrost.aggregate_signatures(
            msg,
            signs,
            Utils.code_to_pub(agregated_nonces[0]),
