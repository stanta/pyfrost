...
def key_generate(nodes, threshold, private_key=None):
    ...
    for node in nodes:
        key_shares.append(
            {
                "id": node,
                "key": sum(f_share[node]) * n_inverse,
...
class Test(unittest.TestCase):
    def test_interpolation(self):
        ...
        self.assertEqual(
            number_errors,
            0,
            f"Testing Interpolation Functions : {number_errors} errors from {len(test_list)}",
        )
    ...
    def test_key_gen_logic(self):
        ...
        for test in test_list:
            ...
            assert (
                generated_key1 == generated_key2
            ), "ERROR: reconstruct key is mismatched"
            if generated_key1 != test:
                number_of_errors = number_of_errors + 1
        self.assertEqual(
            number_of_errors,
            0,
            f"Testing Key Generation Logic : {number_of_errors} errors from {len(test_list)}",
        )
    ...
    def test_stinson_sign_logic(self):
        ...
        for id in range(5):
            ...
            aggregated_signature = Utils.schnorr_aggregate_signatures(
                threshold,
                [s["sign"] for s in signates_subset],
                [s["index"] for s in signates_subset],
            )
            self.assertTrue(
                Utils.schnorr_verify(public_key, message_hash, aggregated_signature),
        ...
        for _ in range(5):
            ...
            for id in nodes_subset:
                ...
                private_nonces.append(
                    {
                        "id": int(id),
                        "public_nonce_d": nonce_d,
                        "public_nonce_e": nonce_e,
                    }
                )
                nonces_dict[id] = {
                    "id": int(id),
